...
function sortPoolsByName(objects) {
    ...
    for (let i = 0; i < sortedArray.length; i++) newObject[sortedArray[i][0]] = sortedArray[i][1];
    return newObject;
}
...
function sortWorkersByHashrate(a, b) {
    if (a.hashrate === b.hashrate) return 0;
...
class Stats {
        ...
        (async () => {
            try {
                ...
                for (let i = 0; i < replies.length; i++) {
                    ...
                    if (stats.global && stats.global.workers > 0) this.statHistory.push(stats);
                }
                this.statHistory = this.statHistory.sort((a, b) => a.time - b.time);
                this.statHistory.forEach((stats) => this.addStatPoolHistory(stats));
            } catch (err) {
                if (this.logger && this.logger.error) this.logger.error(this.logSystem, 'Historics', `Error when trying to grab historical stats ${JSON.stringify(err)}`);
            }
        })();
    }
    ...
    getCoins(cback) {
        if (this.redisClients[0]) this.stats.coins = this.redisClients[0].coins;
        cback();
    }
            ...
            try {
                ...
                for (let idx = 0; idx < pools.length; idx++) {
                    ...
                    for (const i in result[1]) {
                        if (Math.abs(i % 2) != 1) workerName = String(result[1][i]);
                        else sharesLocal += parseFloat(result[1][i]);
                    }
                    if (sharesLocal > 0) totalShares = sharesLocal;
                }
                if (totalShares > 0 || (pindex >= Object.keys(this.stats.pools || {}).length)) { cback(totalShares); return; }
                cback(0);
            } catch (err) { cback(0); }
        })();
    }
    ...
    getBalanceByAddress(address, cback) {
        ...
        (async () => {
            try {
                ...
                for (let idx = 0; idx < pools.length; idx++) {
                    ...
                    for (const i in pays[1]) {
                        if (Math.abs(i % 2) != 1) { workerName = String(pays[1][i]); workers[workerName] = (workers[workerName] || {}); }
                        else { paidAmount = parseFloat(pays[1][i]); workers[workerName].paid = util.coinsRound(paidAmount, coinPrecision); totalPaid += paidAmount; }
                    }
                    for (const b in bals[1]) {
                        if (Math.abs(b % 2) != 1) { workerName = String(bals[1][b]); workers[workerName] = (workers[workerName] || {}); }
                        else { balAmount = parseFloat(bals[1][b]); workers[workerName].balance = util.coinsRound(balAmount, coinPrecision); totalHeld += balAmount; }
                    }
                    for (const b in pends[1]) {
                        if (Math.abs(b % 2) != 1) { workerName = String(pends[1][b]); workers[workerName] = (workers[workerName] || {}); }
                        else { pendingAmount = parseFloat(pends[1][b]); workers[workerName].immature = util.coinsRound(util.satoshisToCoins(pendingAmount, magnitude, coinPrecision), coinPrecision); totalImmature += pendingAmount; }
                    }
                    for (const w in workers) balances.push({ worker: String(w), balance: workers[w].balance, paid: workers[w].paid, immature: workers[w].immature });
                }
            ...
            } catch (err) { cback('There was an error getting balances'); }
        })();
    }
        ...
        (async () => {
            try {
                ...
                for (let idx = 0; idx < pools.length; idx++) {
                    ...
                    const pays = await hscanAsync(`${coin}:payouts`, 0, 'match', `${a}*`, 'count', 50000);
                    const workers = {};
                    for (let i = 0; i < pays[1].length; i += 2) { const workerName = String(pays[1][i]); const paidAmount = parseFloat(pays[1][i + 1]); workers[workerName] = workers[workerName] || {}; workers[workerName].paid = util.coinsRound(paidAmount, coinPrecision); }
                    for (let j = 0; j < bals[1].length; j += 2) { const workerName = String(bals[1][j]); const balAmount = parseFloat(bals[1][j + 1]); workers[workerName] = workers[workerName] || {}; workers[workerName].balance = util.coinsRound(balAmount, coinPrecision); }
                    for (let k = 0; k < pends[1].length; k += 2) { const workerName = String(pends[1][k]); const pendingAmount = parseFloat(pends[1][k + 1]); workers[workerName] = workers[workerName] || {}; workers[workerName].immature = util.coinsRound(pendingAmount, coinPrecision); }
                    for (const worker in workers) poolBalances.push({ pool: poolName, worker: worker, balance: workers[worker].balance || 0, paid: workers[worker].paid || 0, immature: workers[worker].immature || 0 });
                }
                callback(poolBalances);
            } catch (err) { callback('There was an error getting balances'); }
        })();
    }
        ...
        (async () => {
            try {
                for (let rcIndex = 0; rcIndex < this.redisClients.length; rcIndex++) {
                    ...
                    client.coins.map((coin) => { redisCommandTemplates.map((t) => { const clonedTemplates = t.slice(0); clonedTemplates[1] = coin + clonedTemplates[1]; redisCommands.push(clonedTemplates); }); });
                    let replies;
                    try { replies = await runMulti(client.client, redisCommands); } catch (err) { if (this.logger && this.logger.error) this.logger.error(this.logSystem, 'Global', `error with getting global stats ${JSON.stringify(err)}`); throw err; }
                    for (let i = 0; i < replies.length; i += commandsPerCoin) {
                        ...
                        for (let j = replies[i + 10].length; j > 0; j--) { let jsonObj; try { jsonObj = JSON.parse(replies[i + 10][j - 1]); } catch (e) { jsonObj = null; } if (jsonObj !== null) coinStats.payments.push(jsonObj); }
                ...
                Object.keys(allCoinStats).forEach((coin) => {
                    ...
                    coinStats.hashrates.forEach((ins) => {
                        ...
                        if (workerShares > 0) {
                            ...
                            if (worker in coinStats.workers) { coinStats.workers[worker].shares += workerShares; coinStats.workers[worker].diff = diff; if (lastShare > coinStats.workers[worker].lastShare) coinStats.workers[worker].lastShare = lastShare; }
                            else coinStats.workers[worker] = { lastShare: 0, name: worker, diff: diff, shares: workerShares, invalidshares: 0, currRoundShares: 0, currRoundTime: 0, hashrate: null, hashrateString: null, luckDays: null, luckHours: null, paid: 0, balance: 0 };
                            if (miner in coinStats.miners) { coinStats.miners[miner].shares += workerShares; if (lastShare > coinStats.miners[miner].lastShare) coinStats.miners[miner].lastShare = lastShare; }
                            else coinStats.miners[miner] = { lastShare: 0, name: miner, shares: workerShares, invalidshares: 0, currRoundShares: 0, currRoundTime: 0, hashrate: null, hashrateString: null, luckDays: null, luckHours: null };
                        } else {
                            if (worker in coinStats.workers) { coinStats.workers[worker].invalidshares -= workerShares; coinStats.workers[worker].diff = diff; }
                            else coinStats.workers[worker] = { lastShare: 0, name: worker, diff: diff, shares: 0, invalidshares: -workerShares, currRoundShares: 0, currRoundTime: 0, hashrate: null, hashrateString: null, luckDays: null, luckHours: null, paid: 0, balance: 0 };
                            if (miner in coinStats.miners) coinStats.miners[miner].invalidshares -= workerShares; else coinStats.miners[miner] = { lastShare: 0, name: miner, shares: 0, invalidshares: -workerShares, currRoundShares: 0, currRoundTime: 0, hashrate: null, hashrateString: null, luckDays: null, luckHours: null };
                        }
                    });
                    ...
                    if (!portalStats.algos.hasOwnProperty(algo)) portalStats.algos[algo] = { workers: 0, hashrate: 0, hashrateString: null };
                    portalStats.algos[algo].hashrate += coinStats.hashrate;
                    portalStats.algos[algo].workers += Object.keys(coinStats.workers).length;

                    let _shareTotal = 0, _maxTimeShare = 0;
                    for (const worker in coinStats.currentRoundShares) { const miner = worker.split('.')[0]; if (miner in coinStats.miners) coinStats.miners[miner].currRoundShares += parseFloat(coinStats.currentRoundShares[worker]); if (worker in coinStats.workers) coinStats.workers[worker].currRoundShares += parseFloat(coinStats.currentRoundShares[worker]); _shareTotal += parseFloat(coinStats.currentRoundShares[worker]); }
                    for (const worker in coinStats.currentRoundTimes) { const time = parseFloat(coinStats.currentRoundTimes[worker]); if (_maxTimeShare < time) _maxTimeShare = time; const miner = worker.split('.')[0]; if (miner in coinStats.miners && coinStats.miners[miner].currRoundTime < time) coinStats.miners[miner].currRoundTime = time; }
                ...
                if (portalStats.global.workers > 0) {
                    ...
                    for (let i = 0; i < this.statHistory.length; i++) { if (retentionTime < this.statHistory[i].time) { if (i > 0) { this.statHistory = this.statHistory.slice(i); this.statPoolHistory = this.statPoolHistory.slice(i); } break; } }
                    this.redisStats.multi([['zadd', 'statHistory', statGatherTime, this.statsString], ['zremrangebyscore', 'statHistory', '-inf', `(${retentionTime}`]]).exec((err) => { if (err && this.logger && this.logger.error) this.logger.error(this.logSystem, 'Historics', `Error adding stats to historics ${JSON.stringify(err)}`); });
                }

                if (typeof callback === 'function') callback();
            } catch (err) { if (this.logger && this.logger.error) this.logger.error(this.logSystem, 'Global', `error getting all stats${JSON.stringify(err)}`); if (typeof callback === 'function') callback(); }
        })();
    }

    getCoinTotals(coin, options, cb) {
        if (this.stats && this.stats.pools && this.stats.pools[coin]) cb(this.stats.pools[coin]); else cb(null);
    }

    close() {
        try { this.redisClients.forEach((c) => { try { c.client.quit(); } catch (e) { try { c.client.end(true); } catch (e2) { } } }); } catch (e) { }
        try { if (this.redisStats) this.redisStats.quit(); } catch (e) { try { this.redisStats.end(true); } catch (e2) { } }
    }
}
