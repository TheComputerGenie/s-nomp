...
class MasterController {
    ...
    spawnPoolWorkers() {
        ...
        const forks = (() => {
            if (!this.portalConfig.clustering || !this.portalConfig.clustering.enabled) return 1;
            if (this.portalConfig.clustering.forks === 'auto') return os.cpus().length;
            if (!this.portalConfig.clustering.forks || isNaN(this.portalConfig.clustering.forks)) return 1;
    ...
    startCliListener() {
        ...
        listener.on('log', (text) => { this.logger.debug('Master', 'CLI', text); }).on('command', (command, params, options, reply) => {
            switch (command) {
                case 'blocknotify':
                    Object.keys(cluster.workers).forEach((id) => {
                ...
                case 'websiteswitch':
                    ...
                    if (!newDir) { reply('Missing directory parameter for websiteswitch'); break; }
                    ...
                    if (!fs.existsSync(candidate) || !this.hasWebsiteFiles(newDir)) {
                        if (fs.existsSync(defaultDir) && this.hasWebsiteFiles('website')) {
                            ...
                            this.portalConfig.website.directory = 'website';
                        } else {
    ...
    processCoinSwitchCommand(params, options, reply) {
        ...
        const replyError = (msg) => { reply(msg); this.logger.error(logSystem, logComponent, msg); };

        if (!params[0]) { replyError('Coin name required'); return; }
        if (!params[1] && !options.algorithm) { replyError('If switch key is not provided then algorithm options must be specified'); return; }
        else if (params[1] && !this.portalConfig.switching[params[1]]) { replyError(`Switch key not recognized: ${params[1]}`); return; }
        else if (options.algorithm && !Object.keys(this.portalConfig.switching).filter((s) => { return this.portalConfig.switching[s].algorithm === options.algorithm; })[0]) { replyError(`No switching options contain the algorithm ${options.algorithm}`); return; }

        const messageCoin = params[0].toLowerCase();
        const newCoin = Object.keys(this.poolConfigs).filter((p) => { return p.toLowerCase() === messageCoin; })[0];
        if (!newCoin) { replyError(`Switch message to coin that is not recognized: ${messageCoin}`); return; }

        const switchNames = [];
        if (params[1]) switchNames.push(params[1]);
        else {
        ...
        switchNames.forEach((name) => {
            ...
            Object.keys(cluster.workers).forEach((id) => { cluster.workers[id].send({ type: 'coinswitch', coin: newCoin, switchName: name }); });
        });
    ...
    startPaymentProcessor() {
        ...
        for (const pool in this.poolConfigs) {
            ...
            if (enabled) { enabledForAny = true; break; }
        }
        ...
        worker.on('exit', (code, signal) => {
            ...
            setTimeout(() => { this.startPaymentProcessor(); }, 2000);
        });
    }

    startWebsite() {
        if (!this.portalConfig.website.enabled) return;
        ...
        worker.on('exit', (code, signal) => {
            ...
            setTimeout(() => { this.startWebsite(); }, 2000);
        });
    }
