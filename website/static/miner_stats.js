...
function addWorkerToDisplay(name, htmlSafeName, workerObj) {
    ...
    // Start building the worker stats box HTML structure
    htmlToAdd = '<div class="boxStats" id="boxStatsLeft" style="float:left; margin: 9px; min-width: 260px;"><div class="boxStatsList">';

    // Display worker name (extract from HTML-safe name or show "noname")
    if (htmlSafeName.indexOf('_') >= 0) {
        // Extract worker name after the underscore (address_workername format)
    ...
    // Current hashrate with tachometer icon
    ...
    // Average hashrate with tachometer icon
    ...
    // Mining difficulty with shield icon
    ...
    // Total paid amount with money icon
    htmlToAdd += `<div><i class="fa fa-money"></i> <small>Paid: <span id="statsPaid${htmlSafeName}">${workerObj.paid}</span></small></div>`;

    // Time since last share with signal icon
    const lastShareTime = Math.floor(((new Date().getTime()) - (new Date(Math.round(workerObj.lastShare)).getTime())) / 1000);
    htmlToAdd += `<div><i class="fa fa-signal"></i> <small>Last share: <span id="statsLastShare${htmlSafeName}">${lastShareTime}s ago</span></small></div>`;

    // Close the HTML structure
    htmlToAdd += '</div></div></div>';

    // Append the new worker box to the workers container
